...
Runs the model for N games and reports win/loss stats plus per-card
pick/play/burn rates for balance analysis.
...
from env import OABEnv, ACTION_TABLE


def run_evaluation(model, env, num_games):
    """Run games and collect statistics."""
    results = []
    card_stats = defaultdict(lambda: {
        "seen": 0,       # appeared in hand
        "played": 0,     # placed on board
        "burned": 0,     # burned from hand
        "games_played": 0,  # games where this card was played
        "games_won": 0,     # games won where this card was played
    })
    ...
    for game in range(1, num_games + 1):
        ...
        while True:
            ...
            if terminated or truncated:
                ...
                # Update per-card game-level stats
...
def print_summary(results, card_stats, set_id):
    ...
    if card_stats:
        ...
        print(f"{'=' * 50}")
        print(f"  {'Card':<20s} {'Seen':>5s} {'Play':>5s} {'Burn':>5s} {'Play%':>6s} {'WinRate':>8s}")
...
def main():
    ...
    results, card_stats = run_evaluation(model, env, args.num_games)
